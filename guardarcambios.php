    ...
    <div class="row">
        ...
        <div class="col col-lg-8 style='float: none;margin-left: auto;margin-right: auto;'">
            ...
            if(existenick($nick)==false) {
                $sql= "";
                if(verpermiso($_SESSION['nick'],$conexion)==1 || verpermiso($_SESSION['nick'],$conexion)==3) {
                    $sql = "UPDATE clientes SET Nombre='$nombre',nick='$nick',pass='$pass',permiso=$permiso WHERE cod_cliente = $codigo";
                }else{
                    $sql = "UPDATE clientes SET Nombre='$nombre',nick='$nick',pass='$pass' WHERE cod_cliente=$codigo";
                }
                $conexion = conectar();
                $conexion->query($sql);
                ...
                if(verpermiso($_SESSION['nick'],$conexion)==3 || verpermiso($_SESSION['nick'],$conexion)==1){
                    ...
                    echo "Cambios realizados correctamente. Por su seguridad deberá iniciar sesión de nuevo con los nuevos datos";
                    session_destroy();
                ...
                $original = getnick($codigo);
                $conexion = conectar();
                if(mismonick($original,$nick)==true){

                    if(verpermiso($_SESSION['nick'],$conexion)==1 || verpermiso($_SESSION['nick'],$conexion)==3) {
                        $sql = "UPDATE clientes SET Nombre='$nombre',pass='$pass',permiso=$permiso WHERE cod_cliente = $codigo";
                    }else{
                        $sql = "UPDATE clientes SET Nombre='$nombre',pass='$pass' WHERE cod_cliente=$codigo";
                    }


                    $conexion->query($sql);
                    echo "Cambios guardados correctamente";
                    header('refresh:2;url=ver_perfil.php');
                }else{
                    echo "Error, ese nick ya existe en la base de datos";
